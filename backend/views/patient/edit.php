<?php
/**
 * @link      http://www.activemedia.uz/
 * @copyright Copyright (c) 2018. ActiveMedia Solutions LLC
 * @author    Rustam Mamadaminov <rmamdaminov@gmail.com>
 */

use common\models\Patient;
use dosamigos\tinymce\TinyMce;
use frontend\components\View;
use trntv\filekit\widget\Upload;
use yii\helpers\Html;
use yii\helpers\Url;
use yii\web\JsExpression;
use yii\widgets\ActiveForm;
use yii\widgets\Pjax;
use yii2mod\chosen\ChosenSelect;
use nkovacs\datetimepicker\DateTimePicker;

/* @var $this View */
/* @var $model Patient */


$this->title                   = $model->isNewRecord ? __('Create Patient') : $model->fullname;
$this->params['breadcrumbs'][] = ['url' => ['patient/index'], 'label' => __('Manage Patients')];
$this->params['breadcrumbs'][] = $this->title;
$user                          = $this->context->_user();
$this->registerJs('initModule();');
?>
<?php Pjax::begin(['enablePushState' => false]) ?>
<div class="row">
    <?php $form = ActiveForm::begin(['enableAjaxValidation' => false]); ?>
    <div class="col col-md-9">
        <div class="panel panel-primary">
            <div class="panel-heading">
                <h4><?= __('Patient Information') ?></h4>
            </div>
            <div class="panel-body">
                <?= $form->field($model, 'fullname')->textInput(['maxlength' => true]) ?>
                <?= $form->field($model, 'diagnose')->textarea(['rows' => 3])->label() ?>
                <?= $form->field($model, 'address')->textarea(['rows' => 3])->label() ?>
                <?= $form->field($model, 'description')->textarea(['rows' => 6])->label(__('Short Information')) ?>
                <?= $form->field($model, 'content')->widget(TinyMce::class, [
                    'clientOptions' => [
                        'plugins'                 => [
                            "advlist autolink lists link imagetools image charmap print hr anchor pagebreak",
                            "searchreplace wordcount visualblocks visualchars code fullscreen",
                            "insertdatetime media nonbreaking save table contextmenu directionality",
                            "emoticons template paste textcolor colorpicker textpattern",
                        ],
                        'extended_valid_elements' => 'script[language|type|src]',
                        'image_title'             => true,
                        'image_class_list'        => 'img-responsive',
                        'image_dimensions'        => false,
                        'automatic_uploads'       => true,
                        'image_caption'           => true,
                        'content_style'           => 'body {max-width: 768px; margin: 5px auto;}.mce-content-body img{width:98%; height:98%}figure.image{margin:0px;width:100%}',
                        'images_upload_url'       => Url::to(['file-storage/upload', 'type' => 'content-image', 'fileparam' => 'file']),
                        'preview_url'             => Url::to('@frontendUrl/preview/' . $model->getId()),
                        'toolbar1'                => "undo redo | styleselect blockquote | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent | insertfile link image table code fullscreen | media forecolor backcolor | bold italic emoticons",
                    ],
                ])->label(__('Full Information')) ?>

            </div>

        </div>
    </div>
    <div class="col col-md-3 post_settings">
        <div class="panel panel-default">
            <div class="panel-heading">
                <h4><?= __('Settings') ?></h4>
            </div>
            <div class="panel-body">

                <?= $form->field($model, 'status')->widget(ChosenSelect::className(), [
                    'items'         => Patient::getStatusOptions(),
                    'pluginOptions' => ['width' => '100%'],
                ]) ?>

                <div id="widget_published_on_wrapper">

                    <?= $form->field($model, 'birthday')->widget(DateTimePicker::className(), [
                        'type'   => 'date',
                        'locale'           => Yii::$app->language == \common\components\Config::LANGUAGE_UZBEK ? 'uz-latn' : (Yii::$app->language == \common\components\Config::LANGUAGE_CYRILLIC ? 'uz' : 'ru'),
                        'format' => 'dd/MM/yyyy',
                        'clientOptions' => [
                        ],
                    ]) ?>
                </div>
                <?= $form->field($model, '_project')->widget(ChosenSelect::className(), [
                    'items'         => array_merge(['' => ''], \common\models\Project::getProjectsAsOption()),
                    'pluginOptions' => ['width' => '100%', 'allow_single_deselect' => true, 'disable_search' => true],
                ]) ?>
                <?= $form->field($model, 'image')->widget(
                    Upload::className(),
                    [
                        'url'              => ['file-storage/upload'],
                        'acceptFileTypes'  => new JsExpression('/(\.|\/)(jpe?g|png)$/i'),
                        'sortable'         => true,
                        'maxFileSize'      => 5 * 1024 * 1024, // 5 MiB
                        'maxNumberOfFiles' => 1,
                        'clientOptions'    => ['height' => '120%'],
                    ]
                ) ?>
            </div>
            <div class="panel-footer">
                <div class="text-right">
                    <?= Html::submitButton(__('Save'), ['class' => 'btn btn-primary']) ?>
                </div>
                <div class="clearfix"></div>
            </div>
        </div>
    </div>
    <?php ActiveForm::end(); ?>
</div>
<script type="application/javascript">

    function initModule() {
        $('.post_settings').theiaStickySidebar({
            additionalMarginTop: 70,
            additionalMarginBottom: 20
        });
    }

</script>
<?php Pjax::end(); ?>
