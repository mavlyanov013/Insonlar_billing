[![Packagist](https://img.shields.io/packagist/dt/backend\widgets/yii2-file-kit.svg)]()
[![Dependency Status](https://www.versioneye.com/php/backend\widgets:yii2-file-kit/badge.svg)](https://www.versioneye.com/php/backend\widgets:yii2-file-kit/2.0.0)
...
Demo
----
Since file kit is a part of [yii2-starter-kit](https://github.com/backend\widgets/yii2-starter-kit) it's demo can be found in starter kit demo [here](http://backend.yii2-starter-kit.terentev.net/article/create).
...
```
php composer.phar require backend\widgets/yii2-file-kit
```

or add

```
"backend\widgets/yii2-file-kit": "@stable"
```
...
```php
'fileStorage'=>[
    'class' => 'backend\widgets\filekit\Storage',
    'baseUrl' => '@web/uploads'
    'filesystem'=> ...
        // OR
    'filesystemComponent' => ...    
...
```
There are several ways to configure `backend\widgets\filekit\Storage` to work with `flysystem`.
...
- Create a builder class that implements `backend\widgets\filekit\filesystem\FilesystemBuilderInterface` and implement method `build` which returns filesystem object. See ``examples/``
...
public function actions(){
    return [
           'upload'=>[
               'class'=>'backend\widgets\filekit\actions\UploadAction',
...
public function actions(){
    return [
       'delete'=>[
           'class'=>'backend\widgets\filekit\actions\DeleteAction',
...
public function actions(){
    return [
       'view'=>[
           'class'=>'backend\widgets\filekit\actions\ViewAction',
       ]
    ];
}
```
...
Standalone usage
```php
echo \backend\widgets\filekit\widget\Upload::widget([
    'model' => $model,
    'attribute' => 'files',
    'url' => ['upload'],
    'sortable' => true,
...
echo $form->field($model, 'files')->widget(
    '\backend\widgets\filekit\widget\Upload',
    [
        'url' => ['upload'],
        'sortable' => true,
...
```php
 public function behaviors()
 {
    return [
        'file' => [
            'class' => 'backend\widgets\filekit\behaviors\UploadBehavior',
