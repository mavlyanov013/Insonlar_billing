 ...
 * http://github.com/backend\widgets/yii2-file-kit
 *
 ...
 */
(function( $ ) {
    jQuery.fn.yiiUpload = function(options) {
        var $input = this;
        ...
        var methods = {
            ...
            createItem: function(file){
                ...
                if (options.languages && options.useCaption) {
                    options.languages.forEach(function (lang) {
                        var $class = lang === options.language ? 'form-control caption' : 'hidden';
                        console.log(lang);
                        item.append($('<textarea/>', {"name": name + '[caption][' + lang + ']', "text": (typeof file.caption === 'undefined') ? null : file.caption[lang], "class": $class, "placeholder": options.caption}))
                    })
                }
                    item.append($('<span/>', {
                        "class": "name",
                ...
                if ((!file.type || file.type.search(/image\/.*/g) !== -1) && options.previewImage) {
                    item.removeClass('not-image').addClass('image-item image');
                    item.prepend($("<div class='img-wrapper'>").append($('<img/>', {src: file[options.baseUrlAttribute] + '/' +file[options.pathAttribute], "class":"uploaded-image"})));
